...
object ApiDeployment {
  ...
  val cannedResponse: Seq[ApiDeployment] = Seq(
    ApiDeployment("ems-address-weighting-service", Some(Instant.now())),
    ApiDeployment("fake-service-id-1", Some(Instant.now().minusSeconds(1))),
    ApiDeployment("fake-service-id-2", Some(Instant.now().minusSeconds(2)))
  )
